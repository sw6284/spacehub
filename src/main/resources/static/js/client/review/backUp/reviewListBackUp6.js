$(document).ready(function() {
    ...
    const replyTemplate = (data, reviewId) => {
        ...
        $element.append(`<div class="card-body"><p class="card-text">${body}</p><button class="btn btn-danger replyDeleteBtn">삭제</button></div>`);
        $div.append($element);
    ...
    const loadReplies = (reviewId) => {
        $.ajax({
            url: `/review/replyList/${reviewId}`,
            method: 'GET',
            dataType: 'json',
            success: function(replies) {
                if (replies && replies.length > 0) {
                    $.each(replies, function(index, reply) {
                        replyTemplate(reply, reviewId);
                    });
                } else {
            ...
            error: function() {
                console.error("Error retrieving replies for review ID:", reviewId);
            }
        });
    };
    ...
    $(document).off("click", ".replyInsertBtn").on("click", ".replyInsertBtn", function() {
        ...
        $form.find(".replyAuthor").val("admin"); // 작성자 필드를 admin으로 설정
